    ...
    def __getitem__(self, index):
        ...
        if type(violation) is list:
            ...
            if type(_v) is not int:
                raise ValueError("Violation must be a list of integers.(One Hot Encoding Format)")

        tokens_fact = self.tokenizer.encode_plus(
            fact,
        ...
        tokens_fact = self.segmentator.splitEvenly(tokens_fact,segment_size,overlap,max_segment_qtd)
...
def custom_collate_fn(batch):
    ...
    for _ in batch:
        if _['input_ids'].shape[0] < max_length:
            ...
            padding = torch.zeros(padding_qtd,_['input_ids'].shape[1],dtype=torch.int64)
...
class CustomBatchSampler(BatchSampler):
    def __init__(self, dataset, batch_size):
        ...
        self.dataset_sorted = sorted(tqdm(enumerate(self.dataset),desc=f'Sorting data for dinamic batch size'),key=lambda x: x[1]['input_ids'].shape[0],reverse=True)
